...
class EvolutionAgent(BaseAgent):
    ...
    def evolve_hypothesis(self, hypothesis: str, feedback: str, research_goal: str) -> dict:
        """Evolve a single hypothesis based on feedback and the research goal."""
        prompt = f"""
        RESEARCH GOAL: {research_goal}
        ...
        Provide the evolved hypothesis with the same structure as the original (statement, rationale, evidence, assumptions, validation).
        """
        evolution_response = self.get_response(prompt)
        return self._parse_evolved_hypothesis(evolution_response)
